...
def process_archive_hour():
    ...
    if TeleinfoHour.last(TeleinfoHour) == None:
        begin = TeleinfoMinute.first(TeleinfoMinute).timestamp.replace(minute=0, second=0, microsecond=0)

    # else TeleinfoHour is NOT empty then we start with the last TeleinfoHour record (newest)
    else:
        begin = (TeleinfoHour.last(TeleinfoHour).timestamp + timedelta(minutes=60)).replace(minute=0, second=0, microsecond=0)
    
    # we finish with the last TeleinfoMinute record (just 1 hour older than the last one to be sure to have a complete hour)
    end = TeleinfoMinute.last(TeleinfoMinute).timestamp.replace(minute=0, second=0, microsecond=0)
...
#    begin = datetime.now() - timedelta(minutes=3)
    ...
    iinst3 = 0
    # get logs
    logs = TeleinfoMinute.get_between_date(begin, end)
    nb_logs = len(logs)
    ...
    if nb_logs>0:
        ...
        # Save to db
        ret = TeleinfoHour.create(TeleinfoHour, begin, base, papp, iinst1, iinst2, iinst3)
    ...
    logging.config.dictConfig({
        ...
        "handlers": {
            ...
            "info_file_handler": {
                ...
                "level": "INFO",
                "formatter": "simple",
                "filename": os.path.join(logfile_base, 'tiarchive_info.log'),
                "maxBytes": 100000,
                "backupCount": 3,
                "encoding": "utf8"
            },
